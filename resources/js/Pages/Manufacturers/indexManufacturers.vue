...
const destroy = () => {
    formDelete.delete(route('manufacturers.destroy', selectedID.value), {
        preserveScroll: true,
...
const search = () => {
    router.get(route('manufacturers.index', {'buscar': searchText.value}));
};
</script>
...
<template>
    ...
    <MainLayout>
        <template #header>
            ...
            <BaseButton route-name="manufacturers.create" label="+ Marca" color="blue" small />
        </template>
        <template #search>
            ...
            <table>
                ...
                <tbody v-if="table.data">
                    <tr v-for="item in table.data" :key="item.id">
                        <td>{{ item.dsManufacturer }}</td>
                        <td>
                            <div class="inline-flex self-stretch">
                                <BaseButton :href="route('manufacturers.show', item.id)" label="Ver" color="gray" small />
                                <BaseButton :href="route('manufacturers.edit', item.id)" label="Editar" color="green" small />
                                <BaseButton @click="toggleModal(item.id, item.dsManufacturer)" label="Deletar" color="red" small />
                            </div>
                        </td>
                    </tr>
                </tbody>
