...
const data = usePage().props.product;
const currentCategories = data.categories.map(category => category.id);

const form = useForm({
    dsProduct: data.dsProduct,
    price: data.price,
    color: data.color,
    weight: data.weight,
...
const store = () => {
    form.put(route('products.update', data.id), {
        preserveScroll: true,
